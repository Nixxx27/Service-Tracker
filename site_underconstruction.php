...
<?php if ($jrf_tbl->strpriority->Visible) { // strpriority ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onmousedown="ew_Sort(event,'<?php echo $jrf_tbl->SortUrl($jrf_tbl->strpriority) ?>',1);">
			<table cellspacing="0" class="ewTableHeaderBtn"><thead><tr><td align="center"><?php echo "PRIORITY" ?></td><td style="width: 10px;"><?php if ($jrf_tbl->strpriority->getSort() == "ASC") { ?><img src="images/sortup.gif" width="10" height="9" border="0"><?php } elseif ($jrf_tbl->strpriority->getSort() == "DESC") { ?><img src="images/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
...
<?php
	...
	if ($strstatus == "New" || $strstatus == "For GM Approval" || $strstatus == "For VP Approval" || $strstatus == "In-Progress" ||  $strstatus == "Encoded to SAP"	) {
	...
	}elseif ($strstatus == "Finished" || $strstatus == "Cancelled" || $strstatus=="finished" || $strstatus=="cancelled") {
	...
	// 			$warning_icon = "";
	// 			$stroverdue_num = 0;
	// 			echo $overdue_ID=  $jrf_tbl->ID->ListViewValue();
	// 			$overde_id_array = array($overdue_ID);
	// 			$update_overdue = new UpdateClass();
	// 				foreach ($overde_id_array as $value) {
	//    					"$value <br>";
...
<?php
	 $cat  = $jrf_tbl->strcategory->ListViewValue();
	  	switch ($cat) {
		  	case 'Civil Section':
		  		$section_color ="bgcolor='#10a5bb'";
		  		break;
		  	case 'Electrical Section':
		  		$section_color ="bgcolor='#9fad2c'";
		  		break;
		  	case 'Mechanical Section':
		  		$section_color ="bgcolor='#848392'";
		  		break;
		  	default:
		  		$section_color =$color;
		  		break;
