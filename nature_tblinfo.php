...
//
// Table class for nature_tbl
//
class cnature_tbl {
	var $TableVar = 'nature_tbl';
	...
	//
	function cnature_tbl() {
		global $Language;

		// ID
		$this->ID = new cField('nature_tbl', 'nature_tbl', 'x_ID', 'ID', '`ID`', 3, -1, FALSE, '`ID`', FALSE);
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] =& $this->ID;

		// strnature
		$this->strnature = new cField('nature_tbl', 'nature_tbl', 'x_strnature', 'strnature', '`strnature`', 200, -1, FALSE, '`strnature`', FALSE);
		$this->fields['strnature'] =& $this->strnature;

		// strremarks
		$this->strremarks = new cField('nature_tbl', 'nature_tbl', 'x_strremarks', 'strremarks', '`strremarks`', 201, -1, FALSE, '`strremarks`', FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `nature_tbl` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `nature_tbl` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `nature_tbl` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("nature_tblview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "nature_tbladd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("nature_tbledit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("nature_tbladd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("nature_tbldelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=nature_tbl" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
